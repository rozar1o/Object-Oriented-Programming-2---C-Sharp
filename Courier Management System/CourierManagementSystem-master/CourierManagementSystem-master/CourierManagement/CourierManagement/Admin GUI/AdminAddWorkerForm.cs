...
namespace CourierManagement
{
    public partial class AdminAddWorkerForm : Form
    {
        ...
        private Employee setEmployee(DataTable userTable,int id)
        {
        ...
        private void addWorker()
        {
            if(isValidated() && isUnique())
            {
                ...
                DataTable uId = dataAccess.Execute("select * from Users where UserName='" + txtUserName.Text + "'");
                int id = uId.Rows[0].Field<int>("Id");
                if (affectedRowCount > 0)
                {
                    Employee employee = setEmployee(userTable,id);
